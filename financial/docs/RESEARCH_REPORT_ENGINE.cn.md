...
## 6. 方法对比评测(`research.eval`)— 为什么不直接用大模型

把"多智能体引擎 vs 单独用大模型"做成**可度量的评分卡**(当前挂在基金):

- `SingleModelBaseline`:同净值数据、同模型,**一次性 prompt** 出整篇(无黑板/计算/critic/合规;live 模型给 `baseline` role 即无 grounding 纪律)。
- `MethodComparison` / `MethodScorecard`:逐维度对比。**架构保证行**(设计决定的事实):数值来源、可复现、可审计/溯源、跨章节一致、独立复核、职责分离、强制合规;**本次实测行**(从两份真实输出算):合规披露完备、**夏普可核验**(对比确定性 `FundCalc`)、数值一致性门、字数。
- 核心论点:**区别不在文笔(都能写好),在可核验/可复现/可审计/流程合规——银行担责的硬约束**。实测引擎在大多数维度占优;单模型同输入多次运行指标剧烈漂移,引擎恒定可复算。
...
# 实时点亮 Agent、共享黑板、协作流、跨运行经验、报告预览;基金可勾「方法对比」。
...
**已达成(离线可复现、有测试,49/49):** 三条流水线(基金/债券/主题)共享黑板架构;纯 Java 金融计算全单测;黑板单一事实源 + 一致性门 + 有界改稿 + 合规披露;数据 SPI + 容错降级 + 新鲜度 + 溯源;跨运行经验蒸馏/召回;预算/超时/退避/可观测;方法对比评分卡;Web 演示台(三类 + 模型选择 + 可视化)。

**✅ 真实模型验证**:以 GLM Coding Plan(glm-5.2/glm-4.6)端到端跑通基金/债券/主题与方法对比,**0 降级、0 一致性问题**(数字全来自计算)。
