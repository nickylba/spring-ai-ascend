...
> Document-corpus consistency checks for spring-ai-ascend. **93 active gate rules** (canonical bash, executable rule sections counted from `# Rule N — slug` headers), backed by **191 self-tests** (`gate/test_architecture_sync_gate.sh` derives the total at runtime). The canonical numbers live in [`docs/governance/architecture-status.yaml#architecture_sync_gate.baseline_metrics`](../docs/governance/architecture-status.yaml) (single source of truth).
>
