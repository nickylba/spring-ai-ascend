...
| §7.2 + §7.3 | `StatelessEngineExecutor` SPI + `DualTrackRouter` + `SlowPathExecutor` SPI | Already shipped (ADR-0112): `service/engine/spi/StatelessEngine.java`. `DualTrackRouter` planned W2 per Part D; `SlowPathExecutor` implemented by `SyncOrchestrator` + `Checkpointer` per ADR-0021. |
