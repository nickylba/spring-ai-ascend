...
> **Altitude-demotion notice (layer-purity remediation).** This ADR authors the W1 half of L0 section-4 #4 (idempotency; L0 #4 cites "ADR-0027 + ADR-0057"), and its Decision section restates implementation-altitude detail that belongs below L0/L1: the §1 Flyway `V2__idempotency_dedup.sql` DDL and §3 `INSERT … ON CONFLICT (tenant_id, idempotency_key) DO NOTHING` upsert (L3 — SQL/persistence); the §2 `IdempotencyStore.claimOrFind(UUID, UUID, String)` method signature (L7 — method signature); the §4 `409 idempotency_conflict` / `409 idempotency_body_drift` wire codes (L4 — HTTP status); and the §4 filter→`claimOrFind` call chain (L1 — call chain). Per the 2026-05-29 engineering-governance systemic remediation and its round-4 convergence-scan residual, those passages are **demoted to non-authoritative legacy content**: read them as evidence of the historical decision, **not** as live authority. The authoritative homes once migrated are `docs/contracts/*.v1.yaml` (the OpenAPI 409 body + `IdempotencyStore` SPI shape), `architecture/docs/L2/fp-idempotency-claim/` (the `idempotency_dedup` table + `claimOrFind` upsert sequence), and `architecture/facts/generated/code-symbols.json`. This demotion is frozen on the by-adr authority surface as `LPV-ADR-0057-idempotency-sql-spi` in [`docs/adr/normalized/adr-altitude-grandfather.yaml`](normalized/adr-altitude-grandfather.yaml) (the same leaked tokens are also frozen on the L0/L1 prose surface as `LPV-l1-as-idempotency-sql` and on the status-ledger surface as `LPV-SL-idempotency-store-sql-hash-status`), sunset 2026-06-30. What **stays** L0/L1-durable: the `IdempotencyStore` capability identity (a Postgres-backed and an in-memory implementation, both posture-gated), the `(tenant_id, idempotency_key)` key-scope decision, the supersession-in-part of ADR-0027's W1 deferral note, and the enforcer rows E12–E14/E22. A W6 normalization wave authors the per-ADR normalized view (whose `non_authoritative_legacy_content` / `l2_refs` quarantine the detail) or widens `AdrFactExtractor` to this Markdown source, whichever closes the fact-layer gap; this notice is removed when that view lands.
>
