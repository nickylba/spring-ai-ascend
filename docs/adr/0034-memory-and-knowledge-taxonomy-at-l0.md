...
> **Altitude-demotion notice (layer-purity remediation).** This ADR authors L0 section-4 #31 (the Memory and Knowledge Taxonomy), and its Decision section restates implementation-altitude detail that belongs below L0/L1: the "Common Metadata Schema" `public record MemoryMetadata(String tenantId, UUID runId, String sessionId, String source, String ontologyTag, double confidence, Instant expiresAt, String embeddingModelVersion, RedactionState redaction, VisibilityScope visibility) {}` ten-field record signature, and the "GraphMemoryRepository surface enumeration" three-method surface (`addFact` / `query` / `search` with their parameter lists) — both L7 (method/record signature). Per the 2026-05-29 engineering-governance systemic remediation and its round-4 convergence-scan residual, those passages are **demoted to non-authoritative legacy content**: read them as evidence of the historical decision, **not** as live authority. The authoritative homes once migrated are `docs/contracts/*.v1.yaml` (the `MemoryMetadata` envelope + `GraphMemoryRepository` SPI shape) and `architecture/facts/generated/code-symbols.json` (the `code-symbol/...-memory-spi-memorymetadata` field inventory + the `GraphMemoryRepository` method surface). This demotion is frozen on the by-adr authority surface as `LPV-ADR-0034-memory-metadata-record-signature` in [`docs/adr/normalized/adr-altitude-grandfather.yaml`](normalized/adr-altitude-grandfather.yaml), sunset 2026-08-31; it carries the same L7 record-signature category as the sibling rows `LPV-ADR-0028` (CausalPayloadEnvelope) and `LPV-ADR-0030` (Skill SPI records). The canonical `embeddingModelVersion` field name is separately FACT-OWNED + MIGRATED (`docs/governance/remediation-inventory/no-loss-audit.md`) and its contract-catalog reconciliation rides `LPV-ADR-0044`; only the record-signature catalogue authored here is the by-adr leak this notice demotes. What **stays** L0/L1-durable: naming the M1–M6 memory taxonomy as a top-level architectural concept, naming `MemoryMetadata` and `GraphMemoryRepository` as the memory-subsystem boundary identities, the Graphiti-as-W1-reference selection, and the ADR-0051 platform-vs-business ownership forward note. A W6 normalization wave authors the per-ADR normalized view (whose `non_authoritative_legacy_content` / `l2_refs` quarantine the detail) or widens `AdrFactExtractor` to this Markdown source, whichever closes the fact-layer gap; this notice is removed when that view lands.
>
