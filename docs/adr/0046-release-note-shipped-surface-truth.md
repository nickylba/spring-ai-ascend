...
> **Altitude-demotion notice (layer-purity remediation).** This ADR authors L0 section-4 #44 (release-note shipped-surface truth — the §4 Constraint block below), and its Context and Decision sections restate implementation-altitude detail that belongs below L0/L1: the `RunContext` method set `runId`/`tenantId`/`checkpointer`/`suspendForChild` with the forbidden `posture()` token and the "method lists on shipped SPIs must be a subset of the canonical interface" rule (Context P1b, Decision 26b — L7, method signature); the `agent-service/.../orchestration/spi/RunContext.java:16-37` source line-range pointer (Context P1b, References — L1, source/call-chain ref); and the named-test behaviour inventory `OpenApiContractIT` calling `OpenApiSnapshotComparator` vs `ApiCompatibilityTest` being ArchUnit-only (Context P2, Decision 26c — L8, test-class inventory). Per the 2026-05-29 engineering-governance systemic remediation and its round-4 convergence-scan residual (which cites #44 explicitly), those passages are **demoted to non-authoritative legacy content**: read them as evidence of the historical decision, **not** as live authority. The authoritative homes once migrated are `docs/contracts/*.v1.yaml` (the `RunContext` SPI method surface + the OpenAPI snapshot-attribution), `architecture/facts/generated/code-symbols.json` (the `RunContext` `public_methods` at `code-symbol/com-huawei-ascend-bus-spi-engine-runcontext` — `no-loss-audit.md` treats the method set as FACT-OWNED there), `architecture/facts/generated/tests.json` (the `test/<kebab-fqn>` facts for `OpenApiContractIT` / `OpenApiSnapshotComparator` / `ApiCompatibilityTest`), and `architecture/docs/L2/run-http-contract/`. This demotion is frozen on the by-adr authority surface as `LPV-ADR-0046-runcontext-method-list-test-inventory` in [`docs/adr/normalized/adr-altitude-grandfather.yaml`](normalized/adr-altitude-grandfather.yaml), and the live L0 copy is recorded in `docs/governance/remediation-inventory/layer-purity-scan.md` section 1.2 #44, sunset 2026-06-30. What **stays** L0/L1-durable: the release-note shipped-surface-truth invariant itself (every shipped SPI row names real Java symbols; method lists are a subset of the canonical interface; test attributions name the test that performs the asserted check) and the Gate Rule 26 enforcer citation (a D3 citation). A W6 normalization wave authors the per-ADR normalized view (whose `non_authoritative_legacy_content` / `l2_refs` quarantine the detail) or widens `AdrFactExtractor` to this Markdown source, whichever closes the fact-layer gap (this ADR is currently `ledger-only` in `docs/governance/adr-remediation-ledger.yaml` with `normalized_path ""`); this notice is removed when that view lands.
