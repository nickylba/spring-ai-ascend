...
### F-ratchet-promotion-without-dated-baseline — An Advisory Gate Promoted to Changed-Files-Blocking on a Corpus Carrying Historical Findings Hard-Blocks the Whole Corpus Without a Dated Baseline Allow-List

**Status: structurally_addressed** (the changed-files-blocking rung is clean — feature_readiness exits 0 vs origin/main with all 39 historical findings BASELINED; the terminal full-blocking rung is not yet reachable until the dated allow-list is drained to empty.)

One occurrence (progressive-learning-curve-remediation W22, 2026-05-30): promoting Rule 147 / G-30 / E197 (`feature_readiness`) from `--mode advisory` to `--mode changed-files-blocking`. The corpus carried 39 advisory findings on shipped FunctionPoints (33 evidence-axis + 6 decision-axis: 11 internal-channel shipped FPs seeded with structural + decision identity but never wired with `saa.contract_op_refs` / `saa.code_entrypoint_refs` / `saa.test_refs`, and 2 FPs citing an ADR whose normalized view is absent / a `remediation_record`). Because the promotion branch had edited the shared scope surfaces (`function-points.dsl` / `engineering-frames.dsl` / `features.dsl` / the policy file), a naive flip re-scoped all 18 FunctionPoints and BLOCKED on all 39 — the promotion wave's own gate would have been RED.

Root cause: a ratchet gate whose changed-files rung RE-SCOPES the whole corpus on any shared-surface change cannot be promoted to changed-files-blocking on a corpus that still carries KNOWN historical findings without ALSO landing a dated baseline allow-list that freezes those findings — or it behaves as a premature full-blocking against any branch that edits a shared surface. The review's own §13.3 defines the middle rung as "block NEW violations in changed files" (historical debt waits for full-blocking), so a changed-files rung without a baseline tolerance does not implement the rung it claims. The sibling layer-purity gate (Rule 145 / G-27) had solved exactly this with `layer-purity-temporary-violations.yaml`; feature-readiness had no such mechanism. Distinct from `F-layer-gate-flags-delegation-pointer` (a probe too blunt to tell a leak from a delegation pointer — here the findings are genuine, the defect is the missing tolerance machinery) and from `F-numeric-drift` (a baseline-count lockstep defect — here the defect is gate behaviour, though the fix moves one baseline).

Prevention rules (current):

- **Promotion discipline** — before promoting an advisory altitude/readiness/lane gate to changed-files-blocking, run it blocking against the live corpus and, for every pre-existing finding, either discharge it now or freeze it in a dated baseline allow-list keyed to the finding's stable identity with a per-row `sunset_date`. NEVER promote a re-scoping changed-files rung on a corpus with un-baselined historical findings.
- **Baseline allow-list invariants (Rule G-30 / `check_feature_readiness.py`)** — the list is OPTIONAL (absent ⇒ tolerate nothing) but, when present, MUST parse or the gate fails closed; honoured ONLY in changed-files-blocking (full-blocking ignores it); a structural-lie (ownership) finding is never baselinable; every row carries a `sunset_date` and an expired-but-unmatched row is flagged for removal (a NOTE).
- **Self-test parity (Rule 89 / G-5.b)** — a git-scratch ratchet self-test proves the rung blocks an un-baselined finding (rc 1), tolerates a still-open row (rc 0, BASELINED), and blocks again on an expired row (rc 1); plus python unit tests for absent-file, open-row toleration, expiry, full-blocking-ignores-baseline, malformed-fail-closed, and ownership-never-baselined.

Open residual: the changed-files-blocking rung is clean (feature_readiness exits 0 vs origin/main; all 39 findings BASELINED by the 39 FRB- rows; the new self-test + 6 python baseline tests pass). The dated allow-list is debt, not a destination — the terminal full-blocking rung ignores the baseline and would block on all 39. Promoting Rule 147 to full-blocking per ADR-0159 §13.3 requires draining the list: the 6 decision-axis rows (sunset 2026-07-31) by authoring the missing normalized ADR views (ADR-0020 / 0027 / 0056 / 0058) or re-pointing FP-RUN-STATE-TRANSITION off the ADR-0118 remediation_record, and the 33 evidence-axis rows (sunset 2026-08-31) by extracting the entry/test/(contract) facts over the real handlers and wiring the `saa.*_refs`. The list is closed; the sunset dates are the forcing function.
