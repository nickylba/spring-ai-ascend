...
| **RunEvent emissions** (parent side) | `RunCreatedEvent(parent)` → `RunStateTransitionEvent(PENDING→RUNNING)` → `ChildRunSpawnedEvent(childRunId=X, childRunMode=…, joinPolicy=…)` → `SuspendRequestedEvent(suspendReason=AwaitChildren)` + `RunStateTransitionEvent(RUNNING→SUSPENDED)` → … (peer executes child) … → `ChildRunCompletedEvent(childRunId=X, childTerminalStatus=SUCCEEDED)` → `ResumeRequestedEvent(resumeCause=CHILD_COMPLETED)` + `RunStateTransitionEvent(SUSPENDED→RUNNING)` → `RunStateTransitionEvent(RUNNING→SUCCEEDED)` + `TerminalTransitionEvent(SUCCEEDED)`. Peer side emits its own independent RunEvent sequence (starts with its own `RunCreatedEvent(parentRunId=A's runId)`). |
...
| **Test grounding** | `RunHttpContractIT.cancelTerminalReturns409` + `RunHttpContractIT.cancel_route_is_post_not_delete` + `RunHttpContractIT.tenantMismatchReturns403` — confirm the canonical authorization + state-transition shape on the cancel route. |
