...
| `10-governance/` | 目录 | A2D 工作模型内核、目录 catalog、交付物模板、文档约束、评审流程、变更治理、角色边界、双视图模型和阶段契约。 | 管理文档体系本身，约束 AI 与人类协作方式，并提供从版本需求文档到自动实现的集中检查点。 | H1 版本开发边界说明确认 / H2 4+1 架构包确认 / H3 发布例外审核 | 治理文件可以继续按阶段契约、版本需求文档、版本开发边界说明、4+1 架构包、交付计划、基线等子目录展开。 | 过程发现的新文档问题必须沉淀为约束或 catalog 更新；AI 不得把每个需求都升级成人工审批点。 |
| `10-governance/a2d-phases/` | 目录 | AI 阶段契约文件，每个文件定义一个 AI 阶段的操作步骤、输入输出和退出条件。 | 让 AI 按需加载当前阶段的契约，而不是一次性读取完整流程手册。 | AI-1 ~ AI-9 各阶段 | 每个阶段契约使用统一模板：应该做什么、输入、输出、退出条件、何时停下问人、反模式。 | 阶段契约不得与 `CLAUDE.md` 工程规则冲突；升级条件必须与版本开发边界说明一致。 |
...
| `10-governance/a2d-stage-inputs-and-tools.md` | 文件 | 每个检查点的人类必填输入、主产物模板、AI 进入下一阶段使用的工具链、工具输入输出和具体例子。 | 把检查点流程从理念说明变成可启动、可分派、可验收的操作清单。 | H0 ~ H3 所有检查点 / AI-1 ~ AI-9 阶段衔接 | 无下级展开。 | 每个阶段必须说明人类输入、工具用途、工具 I/O、产物模板和进入条件；工具只能投影、生成或校验，不得反向创造架构语义。 |
...
| `10-governance/a2d-artifact-templates.md` | 文件 | 版本需求文档、开发边界说明、4+1 架构包、交付计划、实现证据包、后置投影检查包和基线决策记录的模板。 | 统一各阶段交付物的最低内容，防止把“完善架构稿”当成独立交付。 | H0 ~ H3 所有检查点 / AI-3 / AI-6 / AI-7 | 无下级展开。 | 模板必须要求 allowed / forbidden / legal layer / verification / escalation；非 decision-only 阶段必须有代码、harness、schema、contract test、probe 或漂移检查推进。 |
...
| `10-governance/version-intents/` | 预留目录 | 版本需求文档：版本目标、功能需求、非功能需求、非目标、约束、风险预算和发布门槛。 | 保存人类对版本的集中输入，作为 AI 批量分析和建模的入口。 | H0 人类注入版本需求文档 / AI-1 需求归一化与分流 | 按 `<version>.md` 建档；同一版本的后续版本开发边界说明、4+1 架构包、交付计划必须回链到该文件。 | 不得要求人类先写完整设计；必须明确范围、非目标、约束和发布门槛。 |
| `10-governance/architecture-envelopes/` | 预留目录 | AI 自动推进开发边界、允许变更、禁止变更、兼容性策略、第一批代码 / harness 入口、法律层映射、吸收预算、验证要求和升级条件。 | 定义检查点通过后 AI 可以自动推进的版本级工作合同。 | AI-3 生成版本开发边界说明 / H1 版本开发边界说明确认 | 按 `<version>.md` 建档；必须引用版本需求文档和架构影响矩阵。 | 开发边界外变化必须升级；不得在触发升级条件后继续自动实现；不得只写“完善架构稿”而没有代码 / harness 入口；关键边界必须有机器检查或人工 owner。 |
| `10-governance/review-packets/` | 预留目录 | 4+1 架构包：4+1 视图、结构化元素/关系、决策摘要、契约投影矩阵、harness assertions、自动化边界、开放问题和 H2 裁决。 | 为 H2 提供人类可审阅且可被后续自动化消费的架构包，避免人工阅读全部 A2D 原始文档。 | AI-5 生成 4+1 架构包 / H2 4+1 架构包确认 | 按 `<version>.md` 建档；新文件必须使用 `_TEMPLATE.md`，并回链到产生它的 A2D 已接受产物。 | 不得维护另一套事实；必须能追溯到场景、能力、模块卡、状态矩阵、ICD、不变量、harness 或验证矩阵；自动化边界必须包含检查方法和证据。 |
| `10-governance/delivery-projections/` | 预留目录 | 阶段类型、开发切片、实现任务草案、DoD、代码 / harness / probe 输出、法律层输出、测试计划、自动化投影计划、实现反馈和验证证据索引。 | 为自动实现和开发协作提供可执行交付计划。 | AI-6 生成交付视图 / AI-7 自动实现、文档回写、测试补齐 | 按 `<version>.md` 建档；任务必须能追溯到版本开发边界说明、评审包、实现反馈和验证矩阵。 | 不得生成脱离版本开发边界说明的实现任务；非 decision-only 阶段必须有代码 / harness / schema / contract test / probe / drift check 推进；每个关键任务必须有 DoD、验证方式、liveness guard 和必要的漂移检查。 |
