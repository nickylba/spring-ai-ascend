...
- 真实持久化（Stage 12 JDBC adapter + Flyway + RLS）与真实投递绑定（Stage 15 A2A transport adapter）已落地，Stage 17/18 端到端验证通过（184 tests green）；**仍 deferred**：真实 queue / replay store 物理实现、push vs pull / 是否引 MQ 的最终投递模型裁决（独立 H2/H3 议题）、registry 集成的 resolver 生产实现、连接池治理。
