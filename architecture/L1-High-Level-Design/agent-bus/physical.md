...
当前代码中的 `agent-bus` 是一个 Maven module。它不依赖 `agent-runtime`、`agent-core`、`agent-client`、`agent-middleware` 或 `agent-evolve` 的生产代码。（Stage 17 引入 `agent-runtime` 的 **test-scope** 依赖做端到端集成验证 —— `C3ForwardingEndToEndIntegrationTest` 启动真实 `LocalA2aRuntimeHost`；生产代码边界不变，`AgentBusDependencyBoundaryTest.bus_does_not_depend_on_agent_runtime` 守卫。）
