...
> `AgentCardProvider` 与 `AgentCards` 属于 A2A 协议元数据，位于协议桥包 `engine.a2a`（见 §2.B），不属于本中立 SPI 包。中立 SPI 描述符接口 `AgentCardDescriptor`（提议中，以解耦 A2A spec 依赖）已按 ADR-0162 推迟。
...
| `AgentCardProvider` | interface | A2A Agent Card 供应接口；业务方可选实现，A2A 协议元数据（非中立 SPI） | shipped |
