...
| S2C tenant 事实不一致 | 迁移前目标态要求 `tenantId`，代码未实现。 | Stage 2 已完成契约层迁移（commit `d894f494`）；runtime 构造点待后续波次。 |
