...
/**
 * Structured, dual-mode ops trail on the {@code memopt} logger (ADR-0162 /
...
public final class Slf4jMemoryObserver implements MemoryObserver {
    ...
    private static final Logger LOG = LoggerFactory.getLogger("memopt");
    ...
    public void onOperation(String op, String scope, boolean ok, long latencyMs) {
        ...
        try {
            if (verbose) {
                LOG.info("memopt op={} scope={} ok={} latencyMs={}", op, scope, ok, latencyMs);
            } else {
                LOG.debug("memopt op={} scope={} ok={} latencyMs={}", op, scope, ok, latencyMs);
            }
        } finally {
            MDC.remove("op");
            MDC.remove("scope");
    ...
    public void onDegraded(String op, String scope, String reason) {
        ...
        MDC.put("scope", scope);
        try {
            LOG.warn("memopt degraded op={} scope={} reason={}", op, scope, reason);
        } finally {
            MDC.remove("op");
            MDC.remove("scope");
        }
